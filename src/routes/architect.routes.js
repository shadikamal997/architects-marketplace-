 ...
 * - totalViews: 0 (honest - not tracking yet)
 ...
 * - designs: Performance data per design
 * 
 * No mock data - only real numbers
 */
router.get('/performance', async (req, res) => {
  try {
    ...
    let designsWithRatings = 0;

    const designsPerformance = designs.map(design => {
      const sales = design._count.purchases;
      const reviews = design.reviewCount;
      const rating = design.averageRating;
      ...
      return {
        ...
        downloads: sales, // Alias for sales (purchases = downloads)
        sales: sales,
        rating: rating,
        reviews: reviews,
    ...
    const averageRating = designsWithRatings > 0
      ? Number((totalRatingSum / designsWithRatings).toFixed(2))
      : 0;

    // Find top performing design (by sales)
    const topDesign = designsPerformance.reduce((top, design) => {
      return (!top || design.sales > top.sales) ? design : top;
    }, null);

    return ok(res, {
      metrics: {
        totalViews: 0, // Honest: not tracking yet
        totalDownloads: totalSales, // Downloads = purchases
        ...
        topPerformingDesign: topDesign ? {
          ...
          views: topDesign.views,
          sales: topDesign.sales,
        } : null,
      },
      designs: designsPerformance,
      message: designs.length === 0
        ? 'Create and publish designs to start tracking performance.'
        : totalSales === 0
        ? 'No sales yet. Keep promoting your designs!'
        : null,
    });

  } catch (error) {
    console.error('[Architect] Get performance error:', error);
    return serverError(res, 'Failed to fetch performance data');
  }
});
